...
checkboxInput("no_segments", "Create initial segments table", value = FALSE)
hr()
...
output$tab1 <- renderDataTable({
  ...
  # replace NA with "" for safe comparison
  df1_cmp <- dplyr::mutate_all(df1[1:n, ], ~ ifelse(is.na(.), "", .))
  df2_cmp <- dplyr::mutate_all(df2[1:n, ], ~ ifelse(is.na(.), "", .))

  # row indices where they differ
  diff_rows <- which(apply(df1_cmp != df2_cmp, 1, any))
